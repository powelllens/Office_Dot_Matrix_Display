...
def loadingConfig():
    ...
    config_path = './config/displayconfig.xml'
    ...
    # logo
    activated_modes[4] = str2bool(config.modes.logo.activate.contents[0])
    time_modes[4] = int(config.modes.logo.time.contents[0])
    # hockeytable
...
def Routine_Soccer():
    ...
    while past_time < time_modes[3]:
        ...
        if waldhof_check == False:
            ...
            display.clearoffsetScreen()
            display.drawtext(2,2,"Waldhof Football","8x13B",0,74,193,True)
            display.drawtext(1,18,"worse than 5.","8x13B",5,148,208,True)
            display.updateScreen()
            time.sleep(2)
            break
        ...
        for i in range(0,5):
            if i < 5:
                ...
                display.showimage(160,x + 5,'./soccer/images/' + str(i+1) + '_soccer.png')
                display.updateScreen()
                if not debug:
                    time.sleep(4)
                else:
            ...
            if i < 4:
                for x in range(0,-34,-2):
                    ...
                    display.showimage(160,x + 37,'./soccer/images/' + str(i+2) + '_soccer.png')
...
def Routine_Logo():
    ...
    if debug:
        print 'Print Logo'
    #**********************************************************
    # Programm Part for Logo:
    start_time = time.time()
    past_time = 0.0

    display.clearoffsetScreen()
    display.showimage(0,8,"./images/logo.png")
    display.updateScreen()
